...
class Container:
    ...
    def __setattr__(self, _name, _value):
        if _name == '_liquid_volume' and (
            type(_value) not in (int, float)
        ):
            raise TypeError(
                f"Value can only be a number"
            )
        object.__setattr__(self, _name, _value)
...
class DrinkingView:
    ...
    def choose_volume_liquid(self):
        ...
        while True:
            ...
            if _volume > self.container.liquid_volume:
                print(f"Volume exceeds the required {self.container.liquid_volume} ml")
                continue
            else:
                break
    ...
    def view_drink(self):
        ...
        print(f"{self.thirsty.name.capitalize()} can drink {self.thirsty.thirst_indicator} liquid")
        self.choose_volume_liquid()
        volume_to_drink = self.choose_volume_to_drink()
        self.drinking.drink(volume_to_drink)
        print(f"{self.thirsty.name.capitalize()} can drink {self.thirsty.thirst_indicator} liquid")
        print(f"There is {self.container.liquid_volume} ml of liquid left in the glass")
